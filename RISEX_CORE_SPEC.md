...
## 13. Дифференциация от RISEx — клавиатурный воркфлоу + order flow

После просмотра родного интерфейса rise.trade (полноценный, с TP/SL в форме
ордера, margin-панелью и т.д.) — решили не копировать функциональность, а
специализироваться на двух вещах, используемых непрерывно, а не разово:

1. **Хоткеи** — оказалось, B/S/Esc были только текстовой подсказкой без
   реального обработчика (проверено — `keydown` listener не существовал).
   Теперь реализовано по-настоящему: B/S — Long/Short, 1-5 — плечо
   (1x/2x/5x/10x/25x), Q/W/E/R — 25/50/75/100% от баланса, C — закрыть
   позицию, F — перевернуть (закрыть и тут же открыть в другую сторону,
   такого нет в интерфейсе rise.trade).

2. **Order Flow Imbalance** — соотношение объёма агрессивных покупок/продаж
   за последние 30 сек, считается на клиенте из уже существующего WS-потока
   `channel: trades` (никаких новых эндпоинтов). Индикатор в шапке цены.

## 14. График — тиковая лента вместо минутных баров

По фидбеку: для секундного скальпинга 1-минутный бар — устаревшая на
60 сек информация, структурно не подходит. Переделано:
- **Главный график** — реальные исполненные сделки из WS (`channel: trades`),
  без агрегации по времени, скользящее окно 60 сек, точки красятся по
  стороне агрессора (тот же сигнал, что и Order Flow).
- **OHLC-бары** — второстепенная приглушённая полоса снизу (~25% высоты),
  общий контекст, не главный инструмент.

## 15. Tier 1 Order Book аналитика + онбординг-тултипы

Всё на уже существующем WS-потоке сделок (channel: trades), без новых
эндпоинтов:
- **Heatmap/Hybrid режимы стакана** — интенсивность цвета строки = объём +
  скорость изменения (Dynamic Intensity), золотая пульсация = зона
  дисбаланса >65% в одну сторону.
- **CVD (Cumulative Volume Delta)** — спарклайн + число, нарастающий с
  открытия страницы.
- **VWAP** — считается с открытия страницы, пунктирная линия на тиковом
  графике.
- **Large Order detection** — размер > 3× среднего за последние ~40 сделок →
  золотое кольцо на графике + запись в лог "китов".

**Онбординг-тултипы** — чистый CSS (`[data-tooltip]::after`), без JS,
поэтому работают одинаково для незалогиненного посетителя. Полупрозрачный
чёрный фон + белый текст — намеренно отличается от янтарной темы продукта.
Покрыто: Order Book, режимы стакана, все 3 зоны графика, Order Flow, CVD/VWAP,
плечо, Market/Limit, Balance/Position/Statistics.

Сознательно НЕ реализовано (см. обсуждение): Iceberg Detector, Spoofing
Alert — низкая надёжность эвристик / репутационный риск обвинительных
формулировок без надёжного основания.

## 16. Мультисетевая оплата подписки (Rise / Base / Arbitrum)

Оплата подписки — обычный ERC-20 перевод на EOA-адрес, не завязана на
контракты RISEx (в отличие от торговли) → можно платить с любой сети без
моста. USDC-адреса подтверждены по developer.circle.com:
- Rise: `0xe436820ba0c69702c1d3e601d421c0ef38262739` (fallback, не 100%
  подтверждён отдельно — используется и для баланса кошелька ранее)
- Base: `0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913`
- Arbitrum One (нативный, не мостовой USDC.e): `0xaf88d065e77c8cc2239327c5edb3a432268e5831`

Важный фикс попутно: `checkPaymentStatus()` раньше всегда проверял чек через
Rise-only `ethProvider` — платёж на Base/Arbitrum никогда бы не нашёлся,
подтверждение бы зависло навсегда. Теперь — свой `ethers.JsonRpcProvider` под
сеть, на которой реально отправлена транзакция.

## 17. Защита оплаты подписки от подделки

Раньше: статус подписки хранился только в `localStorage` — правился двумя
строчками в консоли браузера, доступно любому пользователю, не только
разработчику. Плюс проверка платежа смотрела только на `receipt.status===1`
(любая успешная транзакция, не обязательно реальный перевод USDC на наш
адрес), и один и тот же txHash можно было переиспользовать бесконечно.

Исправлено:
- `verifyUsdcTransferInReceipt()` — декодирует Transfer-событие из логов
  ресита, проверяет реальный токен/адрес получателя/сумму, а не просто факт
  успешной транзакции.
- `claimAndSaveSubscription()` — атомарная Firestore-транзакция: документ
  `usedPaymentHashes/{chainId_txHash}` создаётся один раз и больше никогда
  не перезаписывается — закрывает переиспользование одного платежа на
  продление или на несколько аккаунтов.
- Источник правды для статуса `active` — Firestore (`subscriptions/{uid}`),
  не localStorage. Trial-статус остался локальным (низкая цена вопроса).
- См. `FIRESTORE_RULES.md` — обязательные security rules, без них Firestore
  по умолчанию может быть открыт на запись кому угодно.

⚠️ Честный предел: без отдельного бэкенда (Cloud Function) нельзя полностью
исключить запись **выдуманного**, но формально подходящего по формату
txHash напрямую через Firestore SDK — правила Firestore не могут сами
сходить в блокчейн и проверить, что хэш реальный. Подробности и как
закрыть до конца — в `FIRESTORE_RULES.md`.
