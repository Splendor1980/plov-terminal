...
const LANGUAGES = {
    ru: {
        ...
        bubble_p1:           'Быстрый терминал для скальпинга на RISEx (rise.trade). Мы никогда не храним твои средства и не видим ключ твоего основного кошелька — вся торговля подписывается локально, в твоём браузере.',
        ...
        bubble_hint:         'Настройки и это окно доступны в любой момент через ⚙️. Совет: попробуй режим Heatmap/Hybrid у стакана слева — там же живая аналитика CVD/VWAP.',
        bubble_ok:           'Понятно',
        ...
        ob_title:            '📊 Стакан',
        trades_left:         '⚡ Сделки',
        bubble_how_title:    '🚀 Как подключить реальную торговлю:',
        bubble_step1:        '1. На rise.trade → API создай новый API-кошелёк (поле Address оставь пустым) и нажми Authorize',
        bubble_step2:        '2. Убедись, что на твоём аккаунте RISEx есть USDC (Deposit на rise.trade, если ещё нет)',
        bubble_step3:        '3. Вставь приватный ключ этого API-кошелька в поле RISEx API Key справа',
        bubble_step4:        '4. Готово — баланс и позиции теперь настоящие, с биржи',
        bubble_sim_note:     'Никогда не используй ключ основного кошелька (MetaMask/Rabby) — только отдельный API-кошелёк.',
    ...
    en: {
        ...
        bubble_p1:           'A fast, keyboard-first scalping terminal for RISEx (rise.trade) perpetuals. We never custody your funds and never see your main wallet\'s key — every trade is signed locally, in your browser.',
        ...
        bubble_hint:         'This window and all settings are always available via ⚙️. Tip: try Heatmap/Hybrid mode on the order book (left) — live CVD/VWAP analytics included.',
        bubble_ok:           'Got it',
        ...
        ob_title:            '📊 Order Book',
        trades_left:         '⚡ Trades',
        bubble_how_title:    '🚀 Connect real trading:',
        bubble_step1:        '1. On rise.trade → API, create a new API wallet (leave Address empty) and click Authorize',
        bubble_step2:        '2. Make sure your RISEx account has USDC (Deposit on rise.trade if you haven\'t already)',
        bubble_step3:        '3. Paste that API wallet\'s private key into RISEx API Key on the right',
        bubble_step4:        '4. Done — your balance and positions are now real, straight from the exchange',
        bubble_sim_note:     'Never use your main wallet\'s key (MetaMask/Rabby) — only a separate API wallet.',
    ...
    zh: {
        ...
        bubble_p1:           '快速的键盘优先 RISEx（rise.trade）永续合约刮头皮交易终端。我们绝不托管您的资金，也绝不接触您主钱包的私钥——所有交易均在您的浏览器本地签名。',
        bubble_p2:           '请选择安全级别：',
        bubble_simple_desc:  '登录即可交易。适合小额资金。',
        bubble_advanced_desc:'每次签名需要 PIN。适合大额资金。',
        bubble_hint:         '此窗口和所有设置随时可通过 ⚙️ 访问。提示：试试左侧订单簿的 Heatmap/Hybrid 模式——内置实时 CVD/VWAP 分析。',
        bubble_ok:           '明白了',
        bubble_settings:     '去设置',
        ...
        ob_title:            '📊 订单簿',
        trades_left:         '⚡ 成交',
        bubble_how_title:    '🚀 连接真实交易:',
        bubble_step1:        '1. 在 rise.trade → API 创建新的 API 钱包（Address 留空）并点击 Authorize',
        bubble_step2:        '2. 确保您的 RISEx 账户有 USDC（如没有，请在 rise.trade 上 Deposit）',
        bubble_step3:        '3. 将该 API 钱包的私钥粘贴到右侧的 RISEx API Key',
        bubble_step4:        '4. 完成 — 您的余额和持仓现在都是交易所的真实数据',
        bubble_sim_note:     '切勿使用主钱包（MetaMask/Rabby）的私钥 — 仅使用单独的 API 钱包。',
...
Object.assign(LANGUAGES.en, {
    ...
    payment_warning: 'Make sure your wallet has USDC on the selected network. Transaction will be confirmed within 30 seconds.',
...
Object.assign(LANGUAGES.ru, {
    ...
    payment_warning: 'Убедись, что на кошельке есть USDC именно в выбранной сети. Транзакция подтвердится в течение 30 секунд.',
...
Object.assign(LANGUAGES.zh, {
    ...
    payment_warning: '请确保钱包在所选网络上有 USDC。交易将在 30 秒内确认。',
